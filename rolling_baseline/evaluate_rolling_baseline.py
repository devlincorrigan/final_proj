...
def print_summary(rows):
    ...
    actual_pushes = sum(1 for row in rows if row["actual_result"] == "push")
    mean_pick_pushes = sum(1 for row in rows if row["mean_pick"] == "push")
    median_pick_pushes = sum(1 for row in rows if row["median_pick"] == "push")
    print(f"Actual Pushes: {actual_pushes}")
    print(f"Mean Pick Pushes: {mean_pick_pushes}")
    print(f"Median Pick Pushes: {median_pick_pushes}")
    print()
