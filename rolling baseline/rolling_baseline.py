...
DEFAULT_CONFIG = {
    "mapping": str(PROJECT_ROOT / "data/game_id_event_id_bijective.csv"),
    "players": str(PROJECT_ROOT / "data/box_scores/players.csv"),
    "odds_dir": str(PROJECT_ROOT / "data/historical_event_odds_v4"),
    "output": str(SCRIPT_DIR / "rolling_baseline.csv"),
    "window": 40,
    "min_history": 20,
    "limit": 0,
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build a simple NBA player-points baseline from odds and box scores."
    )
    parser.add_argument(
        "--mapping",
    ...
    parser.add_argument(
        ...
        help="Rolling window size for mean/median points.",
    )
    parser.add_argument(
        "--min-history",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Optional cap on the number of mapping rows processed.",
    )
...
def rolling_stats(histories, history_dates, person_id, current_game_date, window):
...
def make_output_row(
    ...
    rolling_mean = statistics.mean(prior_points)
    rolling_median = statistics.median(prior_points)
    line = line_info["line"]
    ...
    return {
        ...
        "rolling_mean_points": f"{rolling_mean:.3f}",
        "rolling_median_points": f"{rolling_median:.3f}",
        "mean_minus_line": f"{(rolling_mean - line):.3f}",
        "median_minus_line": f"{(rolling_median - line):.3f}",
        "actual_points": actual_points,
        "actual_result": (
            "over" if actual_points > line else "under" if actual_points < line else "push"
        ),
        "mean_pick": (
            "over" if rolling_mean > line else "under" if rolling_mean < line else "push"
        ),
        "median_pick": (
            "over"
            if rolling_median > line
            else "under"
            if rolling_median < line
            else "push"
        ),
